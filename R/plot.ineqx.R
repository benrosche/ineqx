...
plot.ineqx <- function(ineqx.out, type, yint=1, xlab=NULL, ylab=NULL, llab=NULL, titl=NULL, xlim=NULL, hline=F, se=F, lowess=F, legend=T, sav=F) {
  ...
  # ineqx.out = out; type="dMuG"; xlab="x"; ylab=c("y1", "y2"); llab=c("a", "b", "c"); titl=c("T1", "T2"); xlim=seq(1,3,1); hline=0; se=F; lowess=F; legend=T; sav=F

  # Checks
  if(!all(names(ineqx.out)==c("vars", "dMu", "dSigma", "dW", "dB", "dD", "dT"))) stop("Must be given an ineqx.out object.")
  if(!stringr::str_detect(type, "dMuP|dMuT|dSigmaP|dSigmaT|dWP|dWT|dBP|dBT|dDP|dDT|dO|dT|dPA")) stop("type must be dMuP|dMuT|dSigmaP|dSigmaT|dWP|dWT|dBP|dBT|dDP|dDT|dO|dT|dPA")
  ...
  } else if(type %in% c("dWP", "dWT", "dBP", "dBT", "dDP", "dDT")) {
...
plot_dWBD <- function(ineqx.out, type=type, yint=1, xlab=NULL, ylab=NULL, llab=NULL, titl=NULL, xlim=NULL, hline=F, se=F, lowess=F, legend=T, sav=F) {
  ...
  # ineqx.out = ineqx.out1; type=substitute(dBG); int=2; timeseq=seq(1985,2020,5); ylab=""; tlab=c("By economic position", "Total"); llab=c("low", "medium", "high"); hline=0; legend=T; sav=F; dim=c(11,5)
...
plot_dPA <- function(ineqx.out, xlab, xlim, sav=F) {
  ...
  p1 <-
    ggplot(ineqx.out$dT[[1]] %>% pivot_longer(cols=c(paste0("d", ystat, "T"), "dW", "dB", "dD", "dO", "dT", "dT+dO"), names_to="name", values_to = "value"),
           aes(x=eval(parse(text = timevar)), y=value, linetype=name, color=name)) +
    ggpubr_ineqx +
    geom_line() +
