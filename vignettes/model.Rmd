...
\delta_B^t &= \sum_j \pi_{jt_0} \big( (\mu_{jt} - \sum_j\pi_{jt}\mu_{jt})^2 - (\mu_{jt_0} - \sum_j\pi_{jt_0}\mu_{jt})^2 \big) \\[10pt]
...
I first entertain the treatment effect framework at a single point in time. Let $D\in\{0,1\}$ be a binary treatment, $Y(D)$ be the potential outcome of the same outcome vector as before, and $\tau=Y(1)-Y(0)$ be the intra-individual causal effect of treatment on the outcome. The group-specific treatment effect on the mean of the treated ($ATT$) then equals the expected value of the differences in the potential outcomes in each group: 
...
Further, the group-specific treatment effect on the group-specific variances ($VTT$) equals the expected value of the difference in the variance in the potential outcomes in each group:
